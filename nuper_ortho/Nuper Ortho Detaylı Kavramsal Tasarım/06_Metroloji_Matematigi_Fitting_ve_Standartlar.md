...
Nuper Ortho, seçilen kafa profiline göre SP25M takılıysa doğrudan dairesel tarama kodu (`SCNPATH`), TP20 takılıysa 2 derinlikte 8-12 diskret temas noktası üretir.
...
- **Nuper Ortho Kuralı:** Temas vektörü ($\vec{V}_{\text{touch}}$), yüzeyin lokal normal vektörüne ($\vec{N}_{\text{surface}}$) daima birebir paralel ve zıt yönde kilitlenir:
...
3. **Doğrulama Sertifikası (Validation Certificate):** Nuper Ortho'nun Rust matematik çekirdeği PTB veri kümeleriyle birim testlere (unit tests) sokulur. Algoritmaların hata payının $10^{-6}\text{ mm}$ (nanometre) altında olduğu belgelenerek müşteriye resmi test raporu olarak sunulur.
