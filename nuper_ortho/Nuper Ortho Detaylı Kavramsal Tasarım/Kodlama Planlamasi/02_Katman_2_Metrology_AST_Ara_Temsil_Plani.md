# 🌳 Katman 2 Planı: Metroloji Ara Temsili / IR (`ortho-ast`)
...
`ortho-ast` kütüphanesi, sistemin **Middle-End / IR (Intermediate Representation)** omurgasıdır. Görevi:
...
Tıpkı bir derleyicinin tip kontrolü (type check) yapması gibi, `ortho-ast` derleme öncesi semantik kuralları denetler:

```rust
// ortho-ast/src/validator.rs
