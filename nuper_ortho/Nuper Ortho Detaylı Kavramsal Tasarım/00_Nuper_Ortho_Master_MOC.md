# 🌐 Nuper Ortho: Otonom CMM ve Metroloji Motoru (Master MOC)
...
**Nuper Ortho**, bu darboğazı kökten çözmek için tasarlanmış; tamamen fabrikadaki yerel bilgisayarda, internet bağlantısı olmadan (**air-gapped**) çalışan, deterministik geometri ayrıştırma ile yerel semantik tolerans okumayı birleştiren **yeni nesil endüstriyel CMM CAM motorudur**.
...
```mermaid
graph LR
    A[STEP CAD Dosyası] --> C[Nuper Ortho Core Engine]
    B[2D PDF Teknik Resim] --> C
    D[CMM / Prob Profili] --> C
    
    subgraph "Nuper Ortho Çekirdeği (Rust + OCCT + Local AI)"
...
Aşağıdaki belgeler, Nuper Ortho'nun kavramsal mimarisinden matematiksel çekirdeğine, fiziksel saha kurallarından post-processor derleyicisine kadar her detayını eksiksiz olarak içerir:
...
Nuper Ortho'nun 5 katmanlı derleyici (compiler) omurgasını hayata geçiren detaylı yazılım mühendisliği planlama notları:
...
| [[01_Katman_1_Ingestion_ve_BRep_Plani\|01. Katman 1: Ingestion & B-Rep]] | `ortho-brep` | OpenCASCADE C++ FFI (`cxx`), analitik yüzeyler, et kalınlığı (<2.5mm). |
| [[02_Katman_2_Metrology_AST_Ara_Temsil_Plani\|02. Katman 2: Metrology AST]] | `ortho-ast` | Nötr Teftiş AST (IR), Datum Frame, semantik AST doğrulayıcı. |
| [[03_Katman_3_Kinematics_ve_Sampling_Plani\|03. Katman 3: Kinematics & Sampling]] | `ortho-kinematics` | PH10 720 diskret açı LUT, MH20i k-means, Gauss/Chebyshev örnekleyici. |
| [[04_Katman_4_Collision_ve_Routing_Plani\|04. Katman 4: Collision & Routing]] | `ortho-router` | +50mm Clearance Box, Keep-Out alanları, prob şaftı sürtünme kontrolü. |
| [[05_Katman_5_Post_Processor_Emitter_Plani\|05. Katman 5: Post-Processor Emitter]] | `ortho-emitter` | Tera/Jinja şablonları, DMIS 5.3, PC-DMIS, `MODE/MAN` ön-hizalama. |
...
| [[17_Disli_Delikler_ve_Karmasik_Unsur_Yonetimi\|17. Dişli Delikler ve Karmaşık Unsurlar]] | `Unsur Sınıflandırma` | Matkap vs nominal çap, vida dişi baypası, yakut bilye koruması, mastar listesi. |
| [[18_CMM_Tezgah_Profili_Prob_ID_ve_Donanim_Lehceleri\|18. CMM Tezgah HAL & Prob ID Eşleme]] | `Donanım HAL` | Strok limitleri, PC-DMIS .prb eşleme, native magazin makroları, Zeiss Calypso ASCII. |
| [[19_Kademeli_AI_Mimarisi_ve_Cok_Sayfali_Kesit_Esleme\|19. Kademeli AI ve Kesit Görünüş Eşleme]] | `Kademeli AI & 2D/3D` | 4 seviyeli AI boru hattı (Tier 0-3), çok sayfalı PDF, kesit A-A hizalama, kaşe temizleme. |
| [[20_Serbest_Yuzey_Profili_ve_Bilesik_Toleranslar\|20. Serbest Yüzey Profili ve İleri GD&T]] | `İleri Metroloji` | Yüzey Profili (Profile of a Surface), ASME Y14.5 PLTZF/FRTZF, eğrilik uyarlamalı örnekleme. |

---

## ⚡ Temel Karşılaştırma Matrisi

| Kriter | Geleneksel CMM Programlama | Hexagon / Zeiss Otomasyonu | Nuper Ortho Çözümü |
|---|---|---|---|
...
Nuper Ortho, Nuper vizyonunun **Physical AI (Fiziksel Yapay Zekâ)** ve **Savunma/İleri İmalat Derin Teknolojisi** dikeyindeki en somut, nakit akışı yaratma potansiyeli en yüksek amiral gemisidir.
