...
const AdminContentForm = ({ type }) => {
        ...
        const fetchContent = async () => {
            ...
            if (slug) {
                ...
                if (!querySnapshot.empty) {
                    ...
                    setInitialContent(docData.content || { blocks: [] });
    ...
    }, [slug, type, db, collectionName]);

    useEffect(() => {
        if (loading || initialContent === null || editorInstanceRef.current) {
            return;
        }
        ...
        const editor = new EditorJS({
            ...
            tools: {
                ...
                quote: { class: Quote, inlineToolbar: true, config: { quotePlaceholder: 'Alıntı metnini girin', captionPlaceholder: 'Alıntıyı yapanı girin' }},
        ...
        return () => {
            if (editorInstanceRef.current && typeof editorInstanceRef.current.destroy === 'function') {
                try {
                    ...
                    console.error('EditorJS yok edilirken hata:', e);
                }
                editorInstanceRef.current = null;
            }
        };
    }, [loading, initialContent, IMGBB_API_KEY]);
    ...
    return (
        <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div><label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1">Başlık <span className="text-red-500">*</span></label><input type="text" name="title" id="title" value={formData.title} onChange={handleChange} className="form-input-std" required /></div>
                <div><label htmlFor="slug" className="block text-sm font-medium text-gray-700 mb-1">URL (Slug) <span className="text-red-500">*</span></label><input type="text" name="slug" id="slug" value={formData.slug} onChange={handleSlugChange} className="form-input-std" required /><p className="text-xs text-gray-500 mt-1">Örn: "yeni-etkinlik-adi"</p></div>
                <div><label htmlFor="date" className="block text-sm font-medium text-gray-700 mb-1">Tarih <span className="text-red-500">*</span></label><input type="text" name="date" id="date" value={formData.date} onChange={handleChange} className="form-input-std" required /><p className="text-xs text-gray-500 mt-1">Örn: "15 Ağustos 2024" veya "Ağustos 2024"</p></div>
                <div><label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Kısa Açıklama (Kartlarda Görünecek)</label><textarea name="description" id="description" rows="3" value={formData.description} onChange={handleChange} className="form-input-std" /></div>
                
                {type === 'event' && (<>
                    <div><label htmlFor="organizer" className="block text-sm font-medium text-gray-700 mb-1">Organizatör</label><input type="text" name="organizer" id="organizer" value={formData.organizer || ''} onChange={handleChange} className="form-input-std" /></div>
                    <div><label htmlFor="location" className="block text-sm font-medium text-gray-700 mb-1">Konum</label><input type="text" name="location" id="location" value={formData.location || ''} onChange={handleChange} className="form-input-std" /></div>
                </>)}
                
                {type === 'bulletin' && (<div><label htmlFor="publisher" className="block text-sm font-medium text-gray-700 mb-1">Yayınlayan</label><input type="text" name="publisher" id="publisher" value={formData.publisher || ''} onChange={handleChange} className="form-input-std" /></div>)}
