...
AutoMetrol, parça sınırlarını ($X_{\min}, X_{\max}, Y_{\min}, Y_{\max}, Z_{\max}$) aldıktan sonra çevresine sanal bir koruma kabuğu örer:
...
AutoMetrol tezgah profilinde bu koordinatlar **"Kalıcı Statik Engel"** olarak tanımlanır. Rota üretilirken bu hacimlerin içine hiçbir şartta yaklaşma veya intikal kodu üretilmez.
...
- **Çarpışma Kontrolü:** AutoMetrol prob modelini (Yakut Bilye + Şaft Silindiri + TP20 Gövde Silindiri) 3 boyutlu birleştirilmiş silindir gövde olarak modeller.
...
AutoMetrol, ölçüm noktalarını yüzey sınırlarından içeriye doğru zorunlu bir güvenlik marjıyla sınırlar:
