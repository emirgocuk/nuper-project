// src/App.js
import React, { useState, useEffect } from 'react'; // useState ve useEffect burada da kullanılacak
...
import { getFirestore, collection, getDocs, orderBy, query } from 'firebase/firestore'; // query ve orderBy eklendi

// Public Bileşen importları (Sizin sağladığınız isimlere göre güncellendi)
import About from './components/About';
import Opportunities from './components/Opportunities'; // Etkinlikler liste sayfası
import Bulletins from './components/Bulletins';     // Bültenler liste sayfası
import OpportunityDetail from './components/EventDetail'; // Etkinlik detay bileşeninizin adı buydu
import BulletinDetail from './components/BulletinDetail'; // Bülten detay bileşeninizin adı buydu

// Admin Bileşen importları (Yeni eklenecek olanlar)
import AdminLogin from './components/AdminLogin';
import AdminPanel from './components/AdminPanel'; // Admin panelinin ana layout'u
import AdminEventsList from './components/admin/AdminEventsList'; // Admin etkinlik listesi
import AdminEventForm from './components/admin/AdminEventForm';   // Admin etkinlik ekleme/düzenleme formu
import AdminBulletinsList from './components/admin/AdminBulletinsList'; // Admin bülten listesi
import AdminBulletinForm from './components/admin/AdminBulletinForm'; // Admin bülten ekleme/düzenleme formu
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <div className="hidden md:flex items-center space-x-4">
            ...
            <Link to="/opportunities" className={`font-sans py-2 ${textColorClass} hover:text-nuper-dark-blue`}>Etkinlikler</Link>
          ...
          {/* Mobil menü ikonunu buraya ekleyebilirsiniz */}
        </div>
      </div>
    </nav>
  );
...
const App = () => {
  ...
  const [loadingEvents, setLoadingEvents] = useState(true); // Etkinlikler için ayrı loading state'i
  const db = getFirestore(app);

  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const q = query(collection(db, "events"), orderBy("createdAt", "desc")); // Son eklenenler önde olsun
      ...
      <div className="min-h-screen font-sans text-gray-900">
        <Routes>
          ...
          <Route element={<MainLayout setExpandedEventId={setExpandedEventId} />}>
            ...
            <Route path="/" element={
              <>
                {/* HomeHeader zaten MainLayout içinde çağırılıyor, burada tekrar çağırmaya gerek yok */}
...
const MainLayout = ({ setExpandedEventId }) => {
  ...
  return (
    <>
      ...
      <Outlet /> {/* İç içe geçmiş rotaların içeriği burada render edilecek */}
      <Footer />
    </>
  );
};
