...
const AdminEventForm = () => {
  ...
  const [croppedAreaPixels, setCroppedAreaPixels] = useState(null); // Bu state, kırpma alanının piksel değerlerini tutar
  ...
  // Kırpma alanı değiştiğinde veya resim yüklendiğinde bu callback tetiklenir
  const onCropComplete = useCallback((croppedArea, newCroppedAreaPixels) => {
    // newCroppedAreaPixels'ın null veya undefined olmadığından emin ol
    if (newCroppedAreaPixels && newCroppedAreaPixels.width > 0 && newCroppedAreaPixels.height > 0) {
      setCroppedAreaPixels(newCroppedAreaPixels);
    } else {
      // Eğer geçerli bir kırpma alanı gelmezse, varsayılan bir değer atayabiliriz
      // Ancak hata mesajını engellemek için, bu durumda butonu devre dışı bırakmak daha güvenli.
      setCroppedAreaPixels(null); // Geçersizse null olarak ayarla
    }
  }, []);
  ...
  const handleCropAndUpload = async () => {
    ...
    setError(null);
    try {
      // croppedAreaPixels'ın geçerli olduğundan emin ol
      if (!imageSrc || !croppedAreaPixels || croppedAreaPixels.width === 0 || croppedAreaPixels.height === 0) {
        throw new Error('Resim veya geçerli bir kırpma alanı seçilmedi. Lütfen resmi kırpma kutusu içinde ayarlayın.');
      }
      ...
      reader.onloadend = async () => {
        ...
        if (result.success && result.data && result.data.url) {
          ...
          // alert yerine daha iyi bir bildirim mekanizması kullanılabilir
          ...
          setCrop({ x: 0, y: 0 });
          setZoom(1);
          setCroppedAreaPixels(null); // Yükleme sonrası kırpma alanını sıfırla
        } else {
  ...
  const insertImage = useCallback(async () => {
    // ImgBB API anahtarının eksik olup olmadığını kontrol et
    if (!IMGBB_API_KEY) {
  ...
  }, [formData.content, IMGBB_API_KEY, ACCEPTED_IMAGE_TYPES, MAX_IMAGE_SIZE_MB]); // Bağımlılıkları ekle
  ...
  return (
    <div className="p-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-4 w-full"> {/* w-full eklendi */}
        <div>
        ...
        <div className="border p-4 rounded-md bg-gray-50 w-full"> {/* w-full eklendi */}
          ...
          />
          {formData.cardImage && (
            <div className="mt-4 flex flex-col items-center card-preview-container w-full"> {/* card-preview-container ve w-full eklendi */}
              <p className="text-sm text-gray-700 mb-2">Mevcut Kart Resmi:</p>
              <img
                src={formData.cardImage}
                alt="Kart Resmi Önizlemesi"
                className="w-full max-w-md object-contain border border-gray-300 rounded-md shadow-sm" // w-full eklendi
              />
        ...
        <div className="border p-4 rounded-md bg-gray-50 w-full"> {/* w-full eklendi */}
          ...
          />
          {formData.mainImage && (
            <div className="mt-4 flex flex-col items-center main-image-preview-container w-full"> {/* main-image-preview-container ve w-full eklendi */}
        ...
        <div className="w-full"> {/* w-full eklendi */}
          ...
          <ReactQuill
            ...
            className="w-full min-h-[200px] border border-gray-300 rounded-lg shadow-sm" // w-full eklendi
          />
          <button
            type="button"
            onClick={insertImage}
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl h-3/4 flex flex-col">
              ...
              <div className="relative flex-grow min-h-[300px]">
                <Cropper
                  ...
                  onCropComplete={onCropComplete} // Bu callback tetiklendiğinde croppedAreaPixels güncellenir
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button
                  ...
                  onClick={() => {
                    ...
                    setCrop({ x: 0, y: 0 });
                    setZoom(1);
                    setCroppedAreaPixels(null); // İptal edildiğinde de sıfırla
                  }}
                ...
                <button
                  ...
                  disabled={imageUploadLoading || !croppedAreaPixels} // croppedAreaPixels null ise butonu devre dışı bırak
                >
